...
create index if not exists idx_recordings_tenant_project_created
  on ${schema}.recordings (tenant_id, project_id, created_at desc);

create table if not exists ${schema}.recording_events (
  recording_event_id text primary key,
  recording_id text not null references ${schema}.recordings (recording_id),
...
create index if not exists idx_recording_events_recording_seq
  on ${schema}.recording_events (recording_id, seq_no asc);

create table if not exists ${schema}.recording_analysis_jobs (
...
create index if not exists idx_recording_analysis_jobs_recording_created
  on ${schema}.recording_analysis_jobs (recording_id, created_at desc);
